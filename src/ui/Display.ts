...
export class Display {
  ...
  toolResult(toolName: string, success: boolean, detail?: string): void {
    ...
    const icon = success
      ...
      : chalk.hex("#F87171")("✘");
    const name = success
      ? chalk.hex("#34D399").bold(toolName)
      : chalk.hex("#F87171").bold(toolName);
    const time = chalk.hex("#475569")(`${ms}ms`);
    const extra = detail
      ? chalk.hex("#64748B")(` — ${detail.slice(0, 80)}`)
      : "";
  ...
  judgePreResult(toolName: string, passed: boolean, reason: string): void {
    ...
    console.log(
      `       ${color.bold(`[PRE-JUDGE ${icon}]`)}  ${chalk.hex("#94A3B8")(`Evaluating args for ${toolName}... ${reason}`)}`,
    );
  }
